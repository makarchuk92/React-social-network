         ...
         <ProfileStatusHooks status={props.status} updateStatus={props.updateStatus}/>
            {editMode ?
             <ProfileDataForm initialValues={props.profile}
              onMainPhotoSelected={onMainPhotoSelected} onSubmit={onSubmit} {...props} /> :
...
const ProfileData = ({...props}) => {
   return <div className={module.accaunt}>
   <div className={module.accaunt__item} >
               <h1>{props.profile.fullName}</h1>
               <img className={module.photo_ava} src={props.profile.photos.large || userPhoto } alt='foto' />
               {props.isOwner && <input type="file" onChange={props.onMainPhotoSelected} className={module.inpit_loading} />}
            </div>
   ...
   <div>
      ...
      return  <ProfileListContacts key={key} contactTitle={key}
       contactValue={props.profile.contacts[key]} />
      }) }
   </div>
   <div className={module.info_text} >
      <div className={module.text_job} >
       <h3>Looking for a job:</h3> <p>{props.profile.lookingForAJob ? "yes" : "no"}</p>  
      </div>
         <div className={module.text_job}>
            <h3>My skills:</h3>
